...
class ShareUserWordFragment : Fragment(R.layout.fragment_share_by_qr_code){
    private var binding: FragmentShareByQrCodeBinding? = null
    private val vm by viewModel<ShareUserWordVm>()
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        if (vm.state.value.isInit.not()){
            vm.sent(ShareUserWordAction.Init(
                userWordId = requireArguments().getString("userWordId") ?: "",
                width = 320.dp.toInt(),
                height =  320.dp.toInt()
            ))
        }
